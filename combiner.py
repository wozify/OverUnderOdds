...
from scoresandoddsscrape_nfl import get_nfl_data
from scoresandoddsscrape_baseball import get_mlb_data
from scoresandoddsscrape_wnba import get_wnba_data
from scoresandoddsscrape_hockey import get_nhl_data
from scoresandoddsscrape_nba import get_nba_data

prizepicks_url = 'https://partner-api.prizepicks.com/projections?league_id='

def create_spreadsheet(sport):
    ...
    # Select a sheet
    worksheet = sh.worksheet(sport)

    odds_df = None
    picks_df = None
    # Get Odds from ScoresAndOdds
    if sport == 'Football':
        odds_df = get_nfl_data()
        picks_df = call_endpoint((prizepicks_url + '9'), include_new_player_attributes=True)
    elif sport == 'Baseball':
        odds_df = get_mlb_data()
        picks_df = call_endpoint((prizepicks_url + '2'), include_new_player_attributes=True)
    elif sport == 'WNBA':
        odds_df = get_wnba_data()
        picks_df = call_endpoint((prizepicks_url + '3'), include_new_player_attributes=True)
    elif sport == 'Hockey':
        odds_df = get_nhl_data()
        picks_df = call_endpoint((prizepicks_url + '8'), include_new_player_attributes=True)
    elif sport == 'Basketball':
        odds_df = get_nba_data()
        picks_df = call_endpoint((prizepicks_url + '7'), include_new_player_attributes=True)
